      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div className="space-y-6">
          <div>
            ...
            <div className="rounded-lg border border-border bg-muted/60 p-5 space-y-3 animate-pulse">
        ...
        <div>
          ...
          <div className="h-[350px] bg-muted/60 rounded-lg animate-pulse" />
        </div>
      </div>
    </div>
  );
...
export default function ContactPage() {
  ...
  const [labName, setLabName] = useState("Fiber Optics Labratory");
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const [contactData, settingsData] = await Promise.all([
        fetchCmsSingleWithAnonFallback("contact-info", async () => {
  ...
  return (
    <div className="container mx-auto px-4 lg:px-8 py-10">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div className="space-y-6">
          <div>
            <h2 className="text-xl font-bold text-primary mb-4">{t("연구실 위치", "Lab Location")}</h2>
            <div className="rounded-lg border border-border bg-card p-5 space-y-3 text-sm text-gray-400">
              ...
              <p><span className="font-semibold text-foreground">{t("사무실", "Office")}:</span> {office}, {building}</p>
              <p><span className="font-semibold text-foreground">{t("연구실", "Lab")}:</span> {lab}, {building}</p>
              <p className="text-gray-500">{address}</p>
            </div>
          </div>
          <div>
            <h2 className="text-lg font-bold text-foreground mb-3">{t("이메일", "Email")}</h2>
            <div className="rounded-lg border border-border bg-card p-5">
              <p className="text-sm text-primary">{emailDisplay}</p>
            </div>
          </div>
        </div>

        <div>
          <h2 className="text-xl font-bold text-primary mb-4">{t("지도", "Map")}</h2>
