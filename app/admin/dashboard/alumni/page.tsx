  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">동문 관리</h1>
      ...
      <div className="flex gap-1.5 mb-5">
        {[{ value: "all", label: "전체" }, ...TYPES].map(t => (
          <button key={t.value} onClick={() => setFilterType(t.value)}
            className={`px-3 py-1.5 rounded-lg text-xs transition-colors ${filterType === t.value ? "bg-destructive/10 text-destructive border border-destructive/20" : "text-gray-500 border border-border hover:text-muted-foreground"}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-1.5">
        {filtered.map(a => (
          <div key={a.id} className="rounded-xl border border-border bg-card p-3.5 flex items-center gap-3">
            ...
            <div className="flex gap-1.5 shrink-0">
              <button onClick={() => setEdit(a)} className="text-gray-500 hover:text-foreground"><Pencil className="h-4 w-4" /></button>
              <button onClick={() => handleDelete(a.id)} className="text-gray-500 hover:text-primary"><Trash2 className="h-4 w-4" /></button>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-bold text-foreground">{edit.id ? "동문 편집" : "동문 추가"}</h2>
              <button onClick={() => setEdit(null)} className="text-gray-500 hover:text-foreground"><X className="h-5 w-5" /></button>
            </div>
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-1.5">이름 <span className="text-primary">*</span></label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-1.5">유형</label>
                <select value={edit.type} onChange={e => setEdit({ ...edit, type: e.target.value })}
                  ...
                  {TYPES.map(t => <option key={t.value} value={t.value} className="bg-sidebar text-foreground">{t.label}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-1.5">연도</label>
            ...
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-1.5">학위</label>
              <input type="text" value={edit.degree} onChange={e => setEdit({ ...edit, degree: e.target.value })} placeholder="Ph.D. / M.Sc."
                className="w-full rounded-lg border border-border bg-muted/60 px-3.5 py-2.5 text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-primary/50 transition-colors" />
            </div>
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-1.5">현재 소속/직책</label>
